    ...
    **model_kwargs,
    ...
    while True:
        ...
        if use_cd:
            ...
            context_start_idx = model_kwargs.get('img_end_idx')+model_kwargs.get("question_len")+model_kwargs.get("prompt_len")
            
            image_attn = outputs.attentions[-1][-1,:,-1,model_kwargs.get('img_start_idx'):model_kwargs.get('img_end_idx')].mean(dim=0).sum()
            context_attn = outputs.attentions[-1][-1,:,-1, context_start_idx:context_start_idx+model_kwargs.get("context_len")].mean(dim=0).sum()
